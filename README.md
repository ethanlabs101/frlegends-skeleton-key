![Header](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/git-images/header.png)

[Getting Started](https://github.com/ethanlabs101/frlegends-skeleton-key/tree/main#getting-started)   •   [Table of Contents](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/TABLE-OF-CONTENTS.md)   •   [Asset Database](https://github.com/ethanlabs101/FRLegends-Asset-Database)   •   [Future Roadmap](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/FUTURE-ROADMAP.md)  •  [Technical Breakdowns](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/TECHNICAL-BREAKDOWNS.md)  •  [Releases](https://github.com/ethanlabs101/frlegends-skeleton-key/releases/tag/v1.0.0)  •  [Directory Structure](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/DIRECTORY-STRUCTURE.md)  •  [How To Update](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/UPDATES.md)  •   [Changelog](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/CHANGELOG.md)  •  [Troubleshooting](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/TROUBLESHOOTING.md)  •  [Contributing](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/CONTRIBUTING.md) 

## What is FR Legends Skeleton Key?

FR Legends Skeleton Key is a powerful toolkit for managing
FR Legends accounts, save data, assets, and customization
through a simple command-line interface.

It provides tools for managing your Vault, working with
player data, installing community assets, managing liveries,
and more — all from one application.

Whether you're looking to manage your own saves, install
custom content, or explore what Skeleton Key can do,
everything is designed to work together through the Vault.

---

### Demo 🎥

![FR Legends Skeleton Key Demo](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/git-images/skeleton-key-demo.gif)

---

### What can it do?

- 🔐 Manage multiple FR Legends accounts
- 💾 Store account data in your local Vault
- 🚗 Manage and inject vehicle assets
- 🎨 Work with custom liveries
- 📦 Browse and download assets from the Online Asset Manager
- 🛠️ Modify supported player data
- 💰 Manage supported game data
- ♻️ Remotely clone and create accounts
- 🔄 Safely update Skeleton Key without overwriting your Vault
- 💻 Run on supported Linux and Termux/Debian environments

***and much more!***

[See full list](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/FEATURES.md)

---

## 📈 v1.0.0 Release Stats

Skeleton Key was officially released 09/18/2026.

Over **1,000 repository clone events** were recorded during the first 14 days, including **300 clone events on release day**.

*Source: GitHub repository traffic analytics*

![Skeleton Key GitHub Traffic](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/git-images/skeleton-key-traffic.png)

---

## Getting Started

New to Skeleton Key?

Follow the guide for your platform below.

### Choose your platform

**Android / Termux**

Install Skeleton Key through the Termux installer.

[View Termux Installation Guide →](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/termux-installation/termux-installation.md)

**Linux**

Install Skeleton Key directly on your Linux system.

[View Linux Installation Guide →](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/docs/linux-installation/linux-installation.md)

---

## FRLMODS.com

Skeleton Key's underlying systems are not limited to the command-line interface.

Skeleton Key's engine is already powering community-facing FR Legends tooling on FRLMods, including the [Add Money & Gold](https://frlmods.com/add-money) tool.

This demonstrates how Skeleton Key's underlying data-processing systems can be integrated into different interfaces and applications.

---

## Disclaimer

FR Legends Skeleton Key is an independent community-developed project and is not affiliated with, endorsed by, or sponsored by FR Legends or its developers.

Use the software at your own risk. Always maintain backups of your account and save data before making changes. You are responsible for how you use the software and for complying with any applicable game, platform, or service terms.

---

## License

- This project is licensed under the Apache License 2.0.
- See the [LICENSE](https://github.com/ethanlabs101/frlegends-skeleton-key/blob/main/LICENSE) file for details.

---

## Thank you!

If you found FR Legends Skeleton Key useful or just want to support,
star this repo & hit that follow button. 

Have a great day. God Bless!

![footer](https://github.com/ethanlabs101/FRLegends-Asset-Database/blob/main/images/footer-animated.svg)

---
